...
export default function VoiceProfilePage() {
  ...
  const followQuery = useQuery({ queryKey: ['voice-follow', voiceQuery.data?.id], queryFn: () => isVoiceFollowing(voiceQuery.data!.id), enabled: Boolean(voiceQuery.data?.id) })
  ...
  if (!voiceQuery.data) return <main className="grid min-h-[60dvh] place-items-center text-center"><div><Headphones className="mx-auto size-10 text-slate-400" /><h1 className="mt-4 text-xl font-bold">Voice não encontrada</h1><Button asChild className="mt-4"><Link to="/app/echoes">Voltar aos Echoes</Link></Button></div></main>
