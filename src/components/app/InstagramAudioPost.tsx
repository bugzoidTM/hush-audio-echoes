...
const InstagramAudioPost = ({
  post
}: InstagramAudioPostProps) => {
  const {
    user
  } = useAuth();
  const {
  ...
  const handleLike = async () => {
    ...
    try {
      if (isLiked) {
        const {
          error
        } = await supabase.from('likes').delete().eq('user_id', user.id).eq('audio_id', post.id);
      ...
      <CardContent className="pt-0 space-y-3">
        ...
        <div className="flex items-center justify-between pt-2">
          <div className="flex items-center space-x-4">
            <Button variant="ghost" size="sm" onClick={handleLike} className={`p-0 h-auto ${isLiked ? 'text-red-500' : ''}`}>
              <Heart className={`w-6 h-6 ${isLiked ? 'fill-current' : ''}`} />
            </Button>
            
            <Button variant="ghost" size="sm" onClick={handleComment} className="p-0 h-auto">
              <MessageCircle className="w-6 h-6" />
            </Button>
          </div>
          
          <Button variant="ghost" size="sm" onClick={handleSave} className={`p-0 h-auto ${isSaved ? 'text-black' : ''}`}>
            <Bookmark className={`w-6 h-6 ${isSaved ? 'fill-current' : ''}`} />
          </Button>
        </div>

        {/* Contagem de curtidas */}
        {likesCount > 0 && <div className="text-sm font-semibold">
            {likesCount} {likesCount === 1 ? 'curtida' : 'curtidas'}
          </div>}

        {/* Link para comentários */}
        <button onClick={handleComment} className="text-sm text-muted-foreground hover:underline">
