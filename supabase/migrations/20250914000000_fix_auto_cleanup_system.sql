-- ================================================================================================
-- MIGRAÇÃO: Correção do Sistema de Limpeza Automática de Áudios
-- Data: 2025-09-14
...
-- 3. Função para executar limpeza manual (pode ser chamada via API)
...
-- 4. Criar índice para otimizar consultas de expiração
CREATE INDEX IF NOT EXISTS idx_audio_posts_expires_at_status 
ON public.audio_posts(expires_at, status) 
WHERE status = 'active';

-- 5. Atualizar posts existentes que já deveriam ter expirado
DO $$
DECLARE
