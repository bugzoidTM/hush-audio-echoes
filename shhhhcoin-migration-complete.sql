-- ================================================
-- MIGRAÇÃO COMPLETA DO SISTEMA SHHHHCOIN
...
-- Tipos de transação
CREATE TYPE IF NOT EXISTS shhhhcoin_transaction_type AS ENUM ('earned', 'spent', 'purchased', 'refunded', 'bonus');

-- Status de convite
CREATE TYPE IF NOT EXISTS referral_status AS ENUM ('pending', 'completed', 'expired', 'cancelled');

-- Status de produto
CREATE TYPE IF NOT EXISTS product_status AS ENUM ('active', 'inactive', 'sold_out', 'discontinued');

-- Status de compra
CREATE TYPE IF NOT EXISTS purchase_status AS ENUM ('pending', 'completed', 'cancelled', 'refunded');
...
('Destaque no Feed', 'Seu próximo áudio aparecerá em destaque no feed', 15.00, 'promotion', 1, false, '{"highlight_duration": 24}'),
('Áudio Estendido', 'Grave áudios de até 5 minutos (em vez de 1 minuto)', 40.00, 'features', 30, false, '{"max_duration": 300}'),
('Badge Verificado', 'Badge especial de verificação no seu perfil', 100.00, 'badges', null, true, '{"badge_type": "verified"}'),
('Histórico Estendido', 'Veja seus áudios por até 7 dias (em vez de 1 dia)', 30.00, 'features', 30, false, '{"history_days": 7}'),
('Notificações Premium', 'Receba notificações push sobre likes e comentários', 20.00, 'features', 30, false, '{"notifications": true}'),
('Pacote Iniciante', 'Combo com filtros básicos + tema escuro', 60.00, 'bundles', 30, false, '{"includes": ["filters_basic", "dark_theme"]}'),
('Pacote Criador', 'Combo completo com todos os recursos premium', 150.00, 'bundles', 30, false, '{"includes": ["all_filters", "themes", "extended_audio", "verified_badge"]}'),
('Boost de Alcance', 'Seus próximos 5 áudios terão maior alcance', 35.00, 'promotion', 7, false, '{"boost_count": 5}')
...
CREATE POLICY "Users can update their own wallet" ON shhhhcoin_wallets
